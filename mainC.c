...
{
    /* Precomputed outputs from the fixed-point rsqrt implementation */
    static const struct {
        uint32_t input;
        uint32_t expected;
    } rsqrt_cases[] = {
    ...
    for (uint32_t i = 0; i < (sizeof(rsqrt_cases) / sizeof(rsqrt_cases[0])); i++) {
        uint32_t observed = fast_rsqrt(rsqrt_cases[i].input);

        if (observed != rsqrt_cases[i].expected) {
            passed = false;

            TEST_LOGGER("  FAILED fast_rsqrt case ");
            print_dec((unsigned long) (i + 1));
    ...
    if (passed)
        TEST_LOGGER("  fast_rsqrt / fast_distance_3d: PASSED\n");
...
int main(void)
{
    ...
    TEST_LOGGER("\n=== C Tests ===\n\n");
    TEST_LOGGER("Test: Tower of Hanoi (C implementation)\n");

    start_ticks = get_ticks();
    start_cycles = get_cycles();
    start_instret = get_instret();

    bool hanoi_passed = run_hanoi_test(hanoi_generate_moves);
    ...
    TEST_LOGGER("\nTest: fast reciprocal square root (rsqrt.c)\n");
